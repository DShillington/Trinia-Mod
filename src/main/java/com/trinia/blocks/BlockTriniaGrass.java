...
public class BlockTriniaGrass extends Block implements IGrowable
{
    public static final PropertyBool SNOWY = PropertyBool.create("snowy");
    private static final String __OBFID = "CL_00000251";

    protected BlockTriniaGrass()
    {
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                worldIn.setBlockState(pos, TriniaBlocks.triniaDirt.getDefaultState());
            }
            else
            {
                if (worldIn.getLightFromNeighbors(pos.up()) >= 9)
                {
                    for (int i = 0; i < 4; ++i)
                    {
                        ...
                        if (iblockstate1.getBlock() == TriniaBlocks.triniaDirt && iblockstate1.getBlock() == TriniaBlocks.triniaDirt && worldIn.getLightFromNeighbors(blockpos1.up()) >= 4 && block.getLightOpacity(worldIn, blockpos1.up()) <= 2)
                        {
                            worldIn.setBlockState(blockpos1, TriniaBlocks.triniaGrass.getDefaultState());
                        }
                    }
                }
            }
    ...
    {
        return Item.getItemFromBlock(TriniaBlocks.triniaDirt);
    }
        ...
        while (i < 128)
        {
            ...
            while (true)
            {
                if (j < i / 16)
                {
                    ...
                    if (worldIn.getBlockState(blockpos2.down()).getBlock() == TriniaBlocks.triniaDirt && !worldIn.getBlockState(blockpos2).getBlock().isNormalCube())
                    {
                        ++j;
                        continue;
                    }

	...
	public static void init() {
		...
		if (newestVersion != null) {
			...
			if (newestVersion.equalsIgnoreCase(currentVersion)) {
				...
				show = true;
				updateStatus = "§c[Trinia Mod] out of date! Your Version: "
						+ currentVersion + " §cLatest Version: "
						+ newestVersion;
				LogWrapper.info("Trinia Mod out of date! Your Version: "
						+ currentVersion + " Latest Version: " + newestVersion);
			}
		} else {
			show = true;
			updateStatus = "[Trinia Mod] Failed to connect to check if update is available!";
			LogWrapper
					.info("Failed to connect to check if update is available!");
		}
	}

...
public class BlockMagicalBed extends BlockDirectional
{
    public static final PropertyEnum PART = PropertyEnum.create("part", BlockMagicalBed.EnumPartType.class);
    public static final PropertyBool OCCUPIED = PropertyBool.create("occupied");
    private static final String __OBFID = "CL_00000198";

    public BlockMagicalBed()
    {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        {
            if (state.getValue(PART) != BlockMagicalBed.EnumPartType.HEAD)
            {
            ...
            if (worldIn.provider.canRespawnHere() && worldIn.getBiomeGenForCoords(pos) != BiomeGenBase.hell)
            {
                if (((Boolean)state.getValue(OCCUPIED)).booleanValue())
                {
                    ...
                    {
                        playerIn.addChatComponentMessage(new ChatComponentTranslation(Reference.MAGIC_BED_OCCUPIED));
                        return true;
                    }
                ...
                else
                {
                    ...
                    {
                        playerIn.addChatComponentMessage(new ChatComponentTranslation(Reference.MAGIC_BED_DAYTIME));
                    }
                    else if (enumstatus == EntityPlayer.EnumStatus.NOT_SAFE)
                    {
                        playerIn.addChatComponentMessage(new ChatComponentTranslation(Reference.MAGIC_BED_UNSAFE));
                    }
    ...
    {
        return state.getValue(PART) == BlockMagicalBed.EnumPartType.HEAD ? null : TriniaItems.MagicalBedItem;
    }
    ...
    {
        if (state.getValue(PART) == BlockMagicalBed.EnumPartType.FOOT)
        {
    ...
    {
        return TriniaItems.MagicalBedItem;
    }

    public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player)
    {
        if (player.capabilities.isCreativeMode && state.getValue(PART) == BlockMagicalBed.EnumPartType.HEAD)
        {
            BlockPos blockpos1 = pos.offset(((EnumFacing)state.getValue(FACING)).getOpposite());
    ...
    public IBlockState getStateFromMeta(int meta)
    {
        ...
        return (meta & 8) > 0 ? this.getDefaultState().withProperty(PART, BlockMagicalBed.EnumPartType.HEAD).withProperty(FACING, enumfacing).withProperty(OCCUPIED, Boolean.valueOf((meta & 4) > 0)) : this.getDefaultState().withProperty(PART, BlockMagicalBed.EnumPartType.FOOT).withProperty(FACING, enumfacing);
    }
